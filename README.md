...
`tools/extract-source.py`. Schema 7 remains deliberately
`status: "incomplete"` and `runtimeReady: false`: Waves A, B, E1, E2a, and the bounded E2b
...
`data/encounter-facts-v0.111.0.json` is a deterministic schema 4 compact projection for
future encounter UI work. It is **not** a runtime input in E2b. The stable
...
- `data/game-v0.111.0-source.json` (schema 7 raw source facts); and
...
- `resolvedAudits` retains the three-lane HP rounding history and its
  non-negative agreement without selecting source/consumer precedence.
- `knownUnknowns` no longer contains the resolved E1 absence reasons,
  `UNKNOWN.INITIAL_STATES`, or `UNKNOWN.HP_ROUNDING_CONFLICT`. It explicitly
  retains event behavior, lifecycle closure, and companion-wide formula/runtime
  contract closure, plus all 18 missing source move titles, incomplete
...
the schema 7 artifact's exact four game input rows. `payloadSha256` is SHA-256
...
contracts remain blockers. E2b therefore does not alter the stable UI default,
