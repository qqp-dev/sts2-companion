...
`tools/extract-source.py`. Schema 9 remains deliberately
`status: "incomplete"` and `runtimeReady: false`: Waves A, B, E1, E2a, E2b, E2c1, and bounded E2c2a
...
remain unchanged through E2c2a.
...
`UNKNOWN.EVENT_BEHAVIOR` intentionally remains. The turn-machine component has a
source-complete resolved audit, while `UNKNOWN.EVENT_SCRIPTED_BEHAVIOR` and
`UNKNOWN.EVENT_LIFECYCLE` preserve E2c2 and lifecycle/timeout/result dependencies.
Architect scripting, Battle Friend timeout/escape/results, and general lifecycle are
not extracted by E2c1.
...
`UNKNOWN.EVENT_SCRIPTED_BEHAVIOR` remains for Architect E2c2b.
`UNKNOWN.EVENT_LIFECYCLE` is narrowed to the Battle timeout producer and escape,
...
`data/encounter-facts-v0.111.0.json` is a deterministic schema 6 compact projection for
future encounter UI work. It is **not** a runtime input in E2c2a. The stable
`/sts2` route still reads only `data/encounters.json`; no file under `src/`
imports the projection or the 12.0 MB source artifact. Generate or verify it
...
- `data/game-v0.111.0-source.json` (schema 9 raw source facts); and
...
the schema 9 artifact's exact four game input rows. `payloadSha256` is SHA-256
...
`incomplete`/false: the aggregate event blocker remains because scripted event
behavior and event lifecycle/timeout/result semantics are unresolved; broader lifecycle
closure and companion-wide formula contracts also remain blockers. E2c2a therefore does not alter the stable UI default,
