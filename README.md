...
`tools/extract-source.py`. Schema 10 remains deliberately
`status: "incomplete"` and `runtimeReady: false`: Waves A, B, E1, E2a, E2b, E2c1,
E2c2a, and bounded E2c2b families are complete only for their declared source denominators. Runtime cutover remains gated inside the staged Wave C.
...
remain unchanged through E2c2b.
...
`data/encounter-facts-v0.111.0.json` is a deterministic schema 7 compact projection for
future encounter UI work. It is **not** a runtime input in E2c2b. The stable
...
- `data/game-v0.111.0-source.json` (schema 10 raw source facts); and
...
the schema 10 artifact's exact four game input rows. `payloadSha256` is SHA-256
...
closure and companion-wide formula contracts also remain blockers. E2c2b therefore does not alter the stable UI default,
