...
`tools/extract-source.py`. Schema 8 remains deliberately
`status: "incomplete"` and `runtimeReady: false`: Waves A, B, E1, E2a, E2b, and bounded E2c1
...
`data/encounter-facts-v0.111.0.json` is a deterministic schema 5 compact projection for
future encounter UI work. It is **not** a runtime input in E2c1. The stable
...
- `data/game-v0.111.0-source.json` (schema 8 raw source facts); and
...
the schema 8 artifact's exact four game input rows. `payloadSha256` is SHA-256
...
closure and companion-wide formula contracts also remain blockers. E2c1 therefore does not alter the stable UI default,
