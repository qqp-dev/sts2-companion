...
3. **Wave C — staged projection and UI migration:** C0 built the checked compact
   ...
   behavior inheritance; E2a now adds initial creature state and intrinsic Power-hook closure without a consumer change. Later E2 slices still precede shadow UI,
...
`tools/extract-source.py`. Schema 6 remains deliberately
`status: "incomplete"` and `runtimeReady: false`: Waves A, B, E1, and the bounded E2a
families are complete only for their declared source denominators. Runtime cutover remains gated inside the staged Wave C.
The app does **not** import this artifact or the E2a compact projection. It still
...
E2a does **not** claim event-turn behavior, downstream HP Decimal-to-integer
conversion/cap/storage, general summon/death/revive/phase/removal lifecycle, or
companion-wide formula inlining. Those remain reason-coded blockers.

## E2a compact encounter projection (offline; no consumer change)

`data/encounter-facts-v0.111.0.json` is a deterministic compact projection for
future encounter UI work. It is **not** a runtime input in E2a. The stable
`/sts2` route still reads only `data/encounters.json`; no file under `src/`
imports the projection or the 11.7 MB source artifact. Generate or verify it
...
- `data/game-v0.111.0-source.json` (schema 6 raw source facts); and
...
- `knownUnknowns` no longer contains the resolved E1 absence reasons or
  `UNKNOWN.INITIAL_STATES`. It explicitly retains event behavior and HP
  conversion/rounding, adds lifecycle closure and companion-wide formula/runtime
  contract closure, and keeps all 18 missing source move titles, incomplete
...
the schema 6 artifact's exact four game input rows. `payloadSha256` is SHA-256
...
`incomplete`/false: event behavior, lifecycle closure, companion-wide formula
contracts, and E2b HP conversion/rounding remain blockers. E2a therefore does not alter the stable UI default,
