...
2. **Wave B — source combat behavior (landed):** move registration/title/intent,
   ...
   graphs.
3. **Wave C — staged projection and UI migration:** C0 builds the checked compact
   projection without a consumer change; E1/E2 extraction gates precede shadow
   UI, source-first cards, and a later QA-approved default switch.
...
their static source scope. Runtime cutover remains gated inside the staged Wave C.
The app does **not** import this artifact or the C0 compact projection. It still
...
## C0 compact encounter projection (offline; no consumer change)

`data/encounter-facts-v0.111.0.json` is a deterministic compact projection for
future encounter UI work. It is **not** a runtime input in C0. The stable
`/sts2` route still reads only `data/encounters.json`; no file under `src/`
imports the projection or the 8.7 MB source artifact. Generate or verify it
...
- `data/game-v0.111.0-source.json` (schema 4 raw source facts); and
...
- `sourceFacts` contains exact source encounter/monster/state identities,
  ...
  behavior owners, and selection graphs. Event encounters remain source-only.
...
- `knownUnknowns` records absent source placement, observed-ID aliases,
  initial-state and event-behavior coverage, the Decimillipede abstract behavior
  inheritance join, all 18 missing source move titles, the audited HP rounding
  conflict, and broader world-model families.
...
the schema 4 artifact's exact four game input rows. `payloadSha256` is SHA-256
...
`runtimeScopes.encounterCompanion` remain `incomplete`/false: E1 source
placement/alias/inheritance extraction and E2 initial-state/event/lifecycle
extraction have not landed. C0 therefore does not alter the stable UI default,
