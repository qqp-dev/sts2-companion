...
`tools/extract-source.py`. Schema 11 remains deliberately
`status: "incomplete"` and `runtimeReady: false`: Waves A, B, E1, E2a, E2b, E2c1,
E2c2a, E2c2b, and bounded E2d1a families are complete only for their declared source denominators. Runtime cutover remains gated inside the staged Wave C.
...
remain unchanged through E2d1a.
...
`data/encounter-facts-v0.111.0.json` is a deterministic schema 8 compact projection for
future encounter UI work. It is **not** a runtime input in E2d1a. The stable
...
- `data/game-v0.111.0-source.json` (schema 11 raw source facts); and
...
- `sourceFacts` contains exact source encounter/monster/state identities,
  ...
  random repeat/float-weight graphs, and compact production-discovery/core-Add
  contracts. Event encounters remain source-only.
...
- `resolvedAudits` retains the three-lane HP rounding history, the event-turn and
  linked-script closures, and the Architect script closure with explicit formula/
  lifecycle boundaries. No audit selects source/consumer precedence.
- `knownUnknowns` no longer contains the resolved E1 absence reasons,
  ...
  retains event behavior, lifecycle closure, explicit pending E2d1b producer
  semantics, and companion-wide formula/runtime contract closure, plus all 18 missing source move titles, incomplete
...
the schema 11 artifact's exact four game input rows. `payloadSha256` is SHA-256
...
closure and companion-wide formula contracts also remain blockers. E2d1a therefore does not alter the stable UI default,
