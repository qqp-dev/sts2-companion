  ...
  return function compile(id, state, mode) {
    ...
    const result = {
      ...
      encounter: {
        ...
        lifecycle: { componentId: source.lifecycle.componentId, factId: source.lifecycle.factId, status: source.lifecycle.status, dependencies: compact(source.lifecycle.dependencies), core: compact(source.lifecycle.core), dispatch: compact(source.lifecycle.dispatch), listenerRegistry: compact(source.lifecycle.listenerRegistry), removal: compact(source.lifecycle.removal), combatTermination: compact(source.lifecycle.combatTermination), runtimeBoundaries: compact(source.lifecycle.runtimeBoundaries) },
        callouts: [],
      ...
      notices: ["Static source mechanics and possibilities; not a next-move prediction.", "Current HP, Block, Powers, intent, turn, phase, and hand are not observed.", "Possible initial bodies, observed body IDs, and produced bodies are separate sets.", "No source-qualified tactical callouts are currently published; [] is not a quota."],
    };
...
export const internals = Object.freeze({ validateProjection, buildStateModelObservationIndex, rosterRecord, rosterModels, jsonValue, compact, payloadDigest, MAX_VIEW_BYTES, SUPPORTED_SCHEMAS });
