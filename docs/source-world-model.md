...
- after mapping current-book compatibility aliases and display states, all 105
  ...
  values;
- the raw build has eight event encounters absent from current retained
  `encounters`; the strict adapter renders them through source-only primary cards.
  Mysterious Knight is a current retained reference/audit record only. Doormaker
  is archive-only and is not a current selector;
