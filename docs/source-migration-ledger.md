...
[`docs/decision-projection.md`](docs/decision-projection.md). Its phone surface uses an evidence-gated `0..N` collection of independently
qualified callouts. A collapsed view may show a ranked subset only with the total
...
At the time of this historical ledger entry, this preserved the E1/E2 gates and
preceded the current C1 shadow runtime.
